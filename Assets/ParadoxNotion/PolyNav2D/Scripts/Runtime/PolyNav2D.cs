...
public class PolyNav2D : MonoBehaviour
{
	...
	void CreatePolyMap (bool generateMaster)
	{
		...
		for (int i = 0; i < navObstacles.Count; i++) {
			...
			obstaclePolys.Add (new Polygon (transformedPoints, invertPolygon));
		}
		...
		if (generateMaster) {
			...
			if (masterCollider is PolygonCollider2D) {
				...
				for (int i = 0; i < polyCollider.pathCount; ++i) {
					...
					masterPolys.Add (new Polygon (inflated, invertPolygon));
					reversed.Clear ();
				}
...
*/
	...
	public struct Polygon
	{
		...
		public Polygon (Vector2[] points, bool invertPolygon = false)
		{
			if (invertPolygon) {
